...
function startReader() {
  ...
  rendition.on("rendered", (section, view) => {
    ...
    /* Track selection via selectionchange */
    doc.addEventListener("selectionchange", () => {
      const sel = doc.getSelection();
      const text = sel?.toString().trim() || "";
      _lastSelectedText = text;
      if (copyShareBtn) {
        copyShareBtn.classList.toggle(
          "hasSelection", text.length > 1
        );
      }
    });
...
/* Track last selected text across
   all rendered iframe instances */
let _lastSelectedText = "";

copyShareBtn.addEventListener("click", async () => {

  const text = _lastSelectedText.trim();
  ...
  /* Show action sheet */
  const existing =
    document.getElementById("copyShareSheet");
  ...
  const bs =
    ...
    "padding:10px 16px;color:inherit;font-size:14px;" +
  ...
  sheet.innerHTML =
    ...
    'color:#888;border-bottom:1px solid #555;' +
    'max-width:260px;overflow:hidden;text-overflow:ellipsis;' +
    'white-space:nowrap;">' + preview + '</div>' +
    '<div style="display:flex;">' +
    '<button id="doCopy" style="' + bs + '">📋 Copy</button>' +
    '<div style="width:1px;background:#555;margin:8px 0;"></div>' +
    '<button id="doShare" style="' + bs + '">📤 Share</button>' +
    '</div>';
  ...
  document.getElementById("doShare")
    .addEventListener("click", async () => {
      sheet.remove();
      if (navigator.share) {
        try {
