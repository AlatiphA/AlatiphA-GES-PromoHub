...
function startReader() {
  ...
  rendition.on("rendered", (section, view) => {
    ...
    function removeSelectionPopup() {
      document.getElementById("selPopup")?.remove();
    }
    ...
    function showSelectionPopup(x, y, text) {
      ...
      const fr = iframe
        ? iframe.getBoundingClientRect()
        : { left: 0, top: 0 };
      const sx = Math.min(fr.left + x, window.innerWidth - 160);
      const sy = Math.max(fr.top + y - 52, 8);
      ...
      const bs = "background:none;border:none;cursor:pointer;" +
        "padding:8px 12px;color:inherit;font-size:13px;" +
        "display:flex;align-items:center;gap:4px;white-space:nowrap;";
      popup.innerHTML =
        '<button id="selCopy" style="' + bs + '">📋 Copy</button>' +
        '<div style="width:1px;background:#888;margin:6px 0;"></div>' +
        '<button id="selShare" style="' + bs + '">📤 Share</button>';
    ...
    function checkSelection() {
      ...
      if (pageText.length > 0 &&
          text.length >= pageText.length - 20) {
        sel.removeAllRanges();
        removeSelectionPopup();
        showToast("Select All is disabled");
        return;
    ...
    /* selectionchange fires instantly when
       browser menu "Select All" is tapped —
       catches it faster than touchend */
    doc.addEventListener("selectionchange", () => {
      ...
      if (
        ...
        /* Small delay so browser finishes
           its own select-all, then we clear */
        setTimeout(() => {
          sel.removeAllRanges();
          removeSelectionPopup();
          showToast("Select All is disabled");
